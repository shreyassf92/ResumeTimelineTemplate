...
const timelinecard = ({ title, duration, org, place, description }) => {

    var n = 0;
    let descList = description.split('.').map(d => {
        if (d !== "")
    ...
    return (
        <div className={Classes.container}>
            ...
            <button className={Classes.expand} onClick={() => { setCount(!isCardOpen) }}>
                {!isCardOpen &&
